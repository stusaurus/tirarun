(() => {
  ...
  function registerAvoid(o, near=false) {
    ...
    } else if (combo === 3 || combo === 7 || combo === 12) {
  ...
  function speed() {
    const eventBoost = eventMode === 'rush' ? 34 : eventMode === 'bonus' ? 12 : 0;
  ...
  function addSpring(x, y=groundY-12, w=52) {
  ...
  function update(dt) {
    ...
    for (const o of objects) {
      ...
      if (!o.used && horizontal && player.vy >= 0 && bottom >= o.y && prevBottom <= o.y + 18) {
        ...
        player.jumps = 1;
        player.squash = .12;
        popText('SUPER JUMP!', player.x + player.w*.8, player.y - 10, '#ffe66b', .75, 17);
  ...
  function drawRunUI() {
    ...
    if (eventMode !== 'normal' || eventBannerTimer > 0) {
      const label = eventMode === 'rush' ? '🌰 KURI RUSH' : eventMode === 'bonus' ? '⭐ BONUS RUN' : eventBanner;
      if (label) {
        ctx.save();
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
