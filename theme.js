(() => {
  ...
  function alphaBounds(img) {
    ...
    const result = maxX < 0 ? {x:0,y:0,w:img.naturalWidth,h:img.naturalHeight} : {
      ...
      w:(maxX-minX+1)/scale, h:(maxY-minY+1)/scale
    };
  ...
  function drawCover(ctx, img, x, y, w, h, alpha=1) {
    ...
    if (srcRatio > dstRatio) {
      ...
      sy = Math.max(0, Math.min(img.naturalHeight-sh, (img.naturalHeight-sh)*.42));
    }
    ctx.save();
  ...
  function drawGroundAsset(ctx, x, y, w, h) {
    ...
    ctx.save();
    ctx.fillStyle = baseDirt[currentStage] || baseDirt[0];
  ...
  proto.fill = function() {
    if (!ready) return Reflect.apply(nativeFill, this, arguments);
